...
int checkRobotSensorRightWalls(struct Robot * robot, struct Wall_collection * head) {
    struct Wall_collection *ptr, *head_store;
    int i;
    double xDir, yDir;
    int robotCentreX, robotCentreY, xTL, yTL;
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle+90)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle+90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorFFWalls(struct Robot * robot, struct Wall_collection * head) {
    struct Wall_collection *ptr, *head_store;
    int i;
    double xDir, yDir;
    int robotCentreX, robotCentreY, xTL, yTL;
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle+50)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle+50)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle+90)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle+90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
    ...
    //Forty Five wall Sensor
    for (i = 0; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/-10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle+50)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/-10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle+50)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
int angle = 0;
int wallfound = 0;
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int right_sensor , int ff_sensor) {
    if ((robot->currentSpeed==0) && (front_left_sensor == 0) || (front_right_sensor == 0)) {
        if (robot->currentSpeed<(MAX_ROBOT_SPEED-7)) {
            robot->direction = UP;
            robot->currentSpeed += DEFAULT_SPEED_CHANGE;
        }
    }
    ...
    if (front_right_sensor >= 1 || front_left_sensor >=3 )
    {
       robot->currentSpeed = -1;
       robot->angle -= DEFAULT_ANGLE_CHANGE;
       wallfound = 1;
    }
    else if (wallfound==1 && ff_sensor==0)
    {
        robot->angle += DEFAULT_ANGLE_CHANGE;
        robot->currentSpeed = 1;
    }
    if(ff_sensor>=1 && right_sensor>=1 && wallfound == 1)
    {
        if (ff_sensor > (right_sensor/0.7660444431))
        {
           robot->angle += .15;
           if (robot->currentSpeed < (MAX_ROBOT_SPEED))
           {
    ...
    if ((right_sensor >= 4 && ff_sensor <= 4))
    {
        robot->angle -= DEFAULT_ANGLE_CHANGE;
    }
    if ((right_sensor >= 3 && ff_sensor >= 3))
    {
        robot->angle -= 4;
    }
    
    if (wallfound==1 && ff_sensor == (right_sensor/0.7660444431) && robot->currentSpeed < (MAX_ROBOT_SPEED-7))
    {
        robot->currentSpeed += DEFAULT_SPEED_CHANGE;
    }
}
